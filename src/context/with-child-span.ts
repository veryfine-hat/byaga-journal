...
export function withChildSpan (name:string) {
     ...
     */
    function higherOrderFunctionGenerator<T extends method>(fn: T) {
        type ResolvedFunctionReturn = Promise<SafeResponse<Awaited<ReturnType<T>>>>
        /**
         ...
         */
        function higherOrderFunction(...args: Parameters<T>): ResolvedFunctionReturn {
            return createSpan(async (): ResolvedFunctionReturn => {
                annotate('name', name);
                try {
                    return { result: (await fn(...args)) as Awaited<ReturnType<T>> };
                } catch (error) {
                    exception(error);
                    return { error };
                }
